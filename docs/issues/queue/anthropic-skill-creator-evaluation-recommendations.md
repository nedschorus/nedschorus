...
**The output viewer is the most concrete reusable external component found in this pass. Treat the benchmark aggregator as a reference requiring adaptation.** This follows the read-for-ideas disposition already recorded in issue [Candidate skill: eval-agent-change — baseline-vs-candidate A/B with trigger cases and raw-count reporting](https://github.com/nedschorus/nedschorus/issues/23).
...
Any adapted aggregation must preserve attempted-run denominators, distinguish an execution failure from a scored failure, derive actual counts, and keep absent token measurements absent. Name baseline and candidate explicitly. issue [Candidate skill: eval-agent-change — baseline-vs-candidate A/B with trigger cases and raw-count reporting](https://github.com/nedschorus/nedschorus/issues/23) also requires raw counts and regression visibility, so an average score cannot be the sole decision.
