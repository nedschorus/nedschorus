...
1. **Red observed → rerun ×2.**
   ...
   - If honest differencing fails to locate the variable: escalate BLOCKED with the paired evidence package — one failing and one passing run, traces, environment, and state snapshots attached. If the user rules the fix ships anyway, the weak-evidence path below applies, recorded as a named residual.
...
## Debugger-first diagnosis (user, 2026-07-31)
