...
- Admission ≠ authorization: every external action (domain purchase, account creation, first post on any platform) needs the user's explicit go, one at a time — the earned-complexity ladder applies to publishing exactly as to automation.
...
The program stays paused by the user's standing deferral; these five requirements additionally gate its FIRST external action, and where they conflict with the admitted body below, these rule:
