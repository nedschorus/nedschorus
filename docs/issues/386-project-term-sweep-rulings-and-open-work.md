...
- **`agent-cli` becomes `agent-binary`.** On 2026-09-18, at item 6 of the MD-skills seat's walk md-skills-seat-open-questions-2026-09-17, he asked how agent-cli differs from agent and ruled the word is `agent-binary`: "sounds like the right word is agent-binary", then "y". The glossary entry and the code rename land the same day, the provenance stamp field `runtime=` keeps its spelling, and the sweep of "runtime" in the remaining files uses `agent-binary`.
...
At the cold-read-research seat he approved renaming the bare word "walk" to `approval-walk` on `docs/nedschorus-wiki/nedschorus-agent-seat-model.md`. The rule says every project term is in the glossary, and the glossary had `walk-minutes` and `walked-approval` but no entry for the walk itself. So the rename as approved produced a word the rule denies is a term.
