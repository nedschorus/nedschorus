...
**Creating an issue.** The author writes the GHI-MD and runs `scripts/ghi-issue-write.py create <path>`. A file not yet filed lives in `docs/issues/queue/` under any name. The tool does the rest:
...
4. **Name and land.** Move the file to `docs/issues/<number>-<slug>.md` and land it on main through the tool's own pull request, which merges under the approving identity issue [Deferred: main-gatekeeper activation shape 2 — a reviewer attached to the gate and an approving identity of its own](https://github.com/nedschorus/nedschorus/issues/357) gives the gatekeeper. Main's required reviews are satisfied by that identity, not bypassed: a raw push stays refused for every tool.
...
**What link-only costs.** Every issue gets a file and a pull request: a two-line issue needs a markdown document and a merge. The tool makes that cheap by merging its own pull requests without a person, but the cost is not zero, and it is the price of one copy of every fact. **The existing corpus is migrated in its own build-slice, after this tool exists:** measured 2026-09-19, 81 open issues of which 20 have a paired file, so 61 need one written. Until an issue is migrated it keeps its prose body and is read as it stands.
...
**Overtaken by the link-only ruling of 2026-09-15.** An issue body is a computed list of links, so it holds nothing that can move ahead of its files. The ruling below is kept for the reader who remembers it.
...
**Overtaken by the link-only ruling of 2026-09-15.** No author performs the cite step: the tool writes the body, and a failure between its steps is resumable. The residual below is kept for the reader who remembers it.
...
> 4. **You are asked to repair a link** — a cross-reference the maintenance sweep found broken. The request states the defect; repair exactly that link and nothing else. Issue edits go through gh as normal; document-side changes are landed through `scripts/ghi-issue-write.py`, whose pull request merges under the gatekeeper's approving identity; a raw push to main is refused. Reply done: \<the repair\>, done: no change needed — \<why\>, or blocked: \<what stopped you\>.
>
> Boundaries:
>
...
>
> Repair exactly this link and nothing else. Issue edits go through gh as normal; document changes are landed through `scripts/ghi-issue-write.py`, whose pull request merges under the gatekeeper's approving identity; a raw push to main is refused. Reply with exactly one of: done: \<the repair\> — done: no change needed — \<why, e.g. the link already resolves\> — or blocked: \<what stopped you\>.
