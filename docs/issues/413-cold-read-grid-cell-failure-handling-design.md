---
status: landed design; built in pull requests 508 (merged 02f4ede, 2026-09-18) and 521 (merged e8698a9, 2026-09-18), with the skill text of section 9; its cold-read-full-run of 2026-09-16 was triaged, and the user's review of that cold-read-full-run closed on 2026-09-17 with eleven rulings, all applied here; tracked in issue [cold-read grid: retry a failed cold-read-cell once, report its cause, and name a runtime that is down](https://github.com/nedschorus/nedschorus/issues/413)
...
- **The cold-read-fast-read already retries.** `scripts/cold-read-fast-read.py` runs its one cold-read-cell through `scripts/cold-read-agy-cell.py`, agent-binary `agy`, and retries it once when it fails, except on exit 64, because "the same invocation would be refused the same way". Its test drives the retry with a stub that fails a set number of first attempts (`fail_first_attempts` in `scripts/cold-read-fast-read-test.py`). The `agy` agent-binary is outside this design.
...
The reason is that causes are read from text, and text can mislead. A cold-read-cell program's log holds the agent-binary's own messages beside the model's text. The Claude limit messages of section 4 arrive on the `claude` agent-binary's standard output, which on a failed attempt also carries whatever the model printed. The Codex agent-binary writes the model's text onto its standard error (recorded in `lift_cell_status_lines` in `scripts/cold-read-grid.py` and in the classifier's comment in `scripts/cold-read-cell-common.py`, issue [cold-read grid: FELL BACK and STRAY WRITE markers fire on text that is not the cell's — two false positives measured 2026-09-02](https://github.com/nedschorus/nedschorus/issues/244)). A model reviewing a document that quotes a limit message could print that message at the start of a line.
...
| `agent-binary-missing` | the program's `OSError` handler ran when it started the agent-binary (`run_model_chain` in `scripts/cold-read-cell-common.py`) | the error text, which says whether the binary was not found or would not run | the user installs or repairs the agent-binary | yes |
...
When a report is absent, the cold-read-grid writes one marker line into every `.md` file in the cold-read-record: the reports that landed, and the reference-check file. It uses the mechanism that writes the target-changed marker (`mark_reports_target_changed` in `scripts/cold-read-grid.py`). That mechanism puts the line after the provenance stamp when there is one, so the stamp stays the first line, and at the top otherwise; this is the top the ruling means.
