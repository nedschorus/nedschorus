...
A **ruling** here means a written, reasoned disposition that you author: the question, the answer, and why. Authoring one is yours. Making it operative over the user's own standing decisions is not — anything that changes the founding plan or agent-instructions text is a draft for his walk, ending 2 below, however finished it reads.

Work on the item the user chose has three possible endings, all of which you can produce without waiting on him:

1. **A ruling written into its durable home.** When more than one home fits, this is the precedence — most specific first, one home per fact, and a pointer rather than a second copy from anywhere else: the governing design document for anything scoped to one system; otherwise the item's issue body; otherwise `docs/nedschorus-wiki/queue/` for reference material meant to be read rather than acted on, which reaches `docs/nedschorus-wiki/` itself only through the user's queue drain.
2. **A draft queued for a walk**, where the ending is a CLAUDE.md line, a change to a standing decision in the founding plan, or any other agent-instructions text: those need the user's walked approval, which you cannot give yourself. Write the proposed text and what it changes into a queue document under `docs/issues/queue/`, tell him it is ready, and stop. That is a complete ending, not a stalled one.
...
- [#31](https://github.com/nedschorus/nedschorus/issues/31) **Review-system requirements**, learned from the review gate in the legacy system at `~/Projects/nedlern` (obsolete; the requirements were carried forward, the machinery was not). It was dormant by ruling until some class of work first required review; that condition has arrived, so it is live: beyond the legacy requirements, its body records that a class of work requires review only where a defined review procedure exists (ruled 2026-07-31), and designates the first such class — agent-instructions and the wiki — with its procedure and guards (ruled 2026-08-04). One of those guards — the gate's check for walked-approval evidence at check-in — was ruled not to be built on 2026-08-17 (`docs/cross-project/main-gatekeeper-design.md` § Resolved), though the issue body still lists it. Yours is the policy: which classes of work require review, and what the evidence must prove. Rule on that, and write it where ending 1 sends it.
