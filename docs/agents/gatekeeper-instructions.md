...
3. **Slice 9 — the check battery**, the mechanical checks that run at check-in. Its owner has been unruled since 2026-08-14: whether this build owns it or the Phase 1 binding in the toolchain plan, `docs/cross-project/nc-python-toolchain-plan.md`, is corrected is the user's ruling. The specification names it only through the same pointer; the question is the slice plan's § Open item 3.
...
Read the specification and the slice plan, check for open pull requests of this seat's work, and confirm the test suite runs green. Then report to the user where the road stands, and propose which slice to start with, and why. Slice 9 is not available to you until he rules its owner.
