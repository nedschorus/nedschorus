...
The standard-package (§2): the design, the component-contract, and the user-rulings file, whose latest ruling on a point governs. On a later check in the design version, your previous notes, the version they rejected, and the initiator's notes: what the initiator did and why, and any finding the initiator did not apply, with the clause that made it wrong (§4). The state-package names each file's path: the two documents sit in `docs/designs/queue/` while no code exists, and in the component's directory once `implementation-writing` has been entered, which is where a redesign's check finds them (§9).
...
4. Check both documents whole, not only what your previous notes named (§4). You may write and run tests of your own to test your theories, so that your notes say exactly what is wrong and why you believe it; keep them under your evidence directory, never in the suite (§6.1, §9).
5. Write your notes: one finding per material defect — one a component-consumer, a writer or a test would meet — each with a failure scenario and, where you have one, a proposed fix; nits under `Nits`; no wording findings, with the one boundary — wording that leaves a promise open, so two readings give two behaviours, is a finding, and a clause that means the right thing badly said is not (§6.1). §6.1 is your reporting rule, and `CLAUDE.md`'s rule for reviewing a pull request is not: you review an artifact, not a pull request, and you report a clause that promises the wrong thing wherever it sits, the design included (§6.1).
6. Emit.
