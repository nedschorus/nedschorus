...
- [#39](https://github.com/nedschorus/nedschorus/issues/39) **memory instrumentation** — echoing memory reads and writes to the session console as they pass through the paths a hook can observe; hooks that remind rather than block, with no context injection. Reads the harness surfaces as recalled context, and files touched by another session or a plain editor, are outside what any hook sees; read the issue for the scope that is actually being asked for.
