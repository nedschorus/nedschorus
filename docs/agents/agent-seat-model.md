...
|---|---|---|
| `gatekeeper` | the remaining work to activate the main-gatekeeper: the walked-approval evidence format, build slice 6, then the credential work | `gatekeeper-instructions.md` |
...
It protects nothing outside git. `~/.claude/handoffs/` and the job scratch directories are shared machine-local state that every seat writes (each seat's tmux session runs on a server of its own since 2026-08-21 — `tmux -L <name>` — so one server crash cannot take the whole fleet down, but the socket is still derived from the name). Two seats using the same *name* would collide there regardless of branches — which is why one name means one seat.
