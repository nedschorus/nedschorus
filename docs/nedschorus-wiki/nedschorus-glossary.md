...
Every project-specific term is a hyphenated phrase, never a bare generic word: agent-seat, not seat. The hyphens mark it as a defined term with a project meaning, so a reader knows to look it up here and an ordinary word keeps its ordinary meaning (user-ruled 2026-09-15). Two kinds of entry are exempt and carry no hyphen: an established initialism such as GHI, and a skill's slash name such as /handoff.
