...
| A subagent's completion arrives later and separately, as a `<task-notification>` record naming its task-id and a `<status>`; one notification reaches the transcript ONE TO THREE times, never four, in one of four combinations of queue enqueue, delivered user turn, attachment copy and queue remove — and a subagent finishing as its session dies gets only the enqueue | three session transcripts, 2026-08-21 to 2026-08-24, grouped on identical notification body; per-combination counts in `task_notification_text` |
