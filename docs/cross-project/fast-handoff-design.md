...
**Subagents, forks included, are the default way to dispatch work.** They report back on their own, the roster records them, the deferral protects them, and their output reaches the commissioning session without anyone watching a pane. (Reconciled 2026-08-29: the roster records only those still working when a recycle strikes; an ended subagent's record is its report and its transcript.)
...
writes no field. The ignition prompt tells the successor to re-commission
...
revisions, ruled on a rendered mock of the ignition prompt: the roster
...
2026-08-29 probe's finding stands unchanged — resume-by-id across a recycle
...
- **Git is the long-term record and the restore-after-problem source (user-ruled 2026-08-02).** Handoffs are operational, machine-local, disposable. File and repo state travel by commit-as-you-go and the queues, not by handoff.
...
- **The queue-status line rides the ignition prompt (user-ruled 2026-08-12).** The #32 rot-visibility duty (queue depth and oldest item, visible to the user) discharged into a log file under detached and headless supervisors; every successor now receives the line with the instruction to surface anything rotting. The console print stays for a watched pane. (Reconciled 2026-08-29 against this change: expired by the user — "Also useless is the reminder there are files in the queues. Thats what queues are for." The line no longer rides the ignition prompt; the supervisor's console print stays, for a watched pane or the log.)
