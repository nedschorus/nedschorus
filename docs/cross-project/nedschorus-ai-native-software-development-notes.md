...
| Persistent state machine | **Not built; previously declined in the current [seat-model document](https://github.com/nedschorus/nedschorus/blob/main/docs/nedschorus-wiki/agent-seat-model.md)** | Reopen explicitly. Build the smallest durable router needed for one human conversation, work-item state, and recovery. Record why the earlier “no state machine” decision changed. |
