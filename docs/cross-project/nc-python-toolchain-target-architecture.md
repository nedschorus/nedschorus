...
- Registry `quality/rules.toml` as v2; `nc-checkin-quality-gate selftest` (fixtures red/green + `cost_budget_ms` p95) is a wave-1 required check. `owner` and `retirement_condition` are consumed by the scheduled repo-health lane (§4).
...
- Wave 1 tools and mypy config as v2, with the override patterns matched to real paths: the checker is the installed package `nc_checkin_quality_gate` (from `tools/nc_checkin_quality_gate/`, declared in `pyproject.toml`); mypy overrides target `nc_checkin_quality_gate.*` and `tests.*` and CI asserts the overrides matched at least one module each (guards silent full-strictness reversion).
...
- **Self-changes** (`NC-SUP002` set incl. `tools/nc_checkin_quality_gate/**`): evaluated by the base evaluator with an independent verdict, as any protected change. The v2 dual-run/auto-promotion machinery is **removed** (it defended against T3 and defended badly); what remains is the record: the ledger marks evaluator-changing commits, and §A's audit replays fixtures against evaluator changes after the fact. Seed-phase evaluator changes are additionally on the human walk by definition (they are protected-path merges).
...
- **Escalation clock:** a finding's `first_seen` is carried in ledger lines (`first_seen_carryover`) from the first run that reported it; `nc-checkin-quality-gate pending` reads it from the working tree's ledger. Pending > 24 h → the runner notifies the human authority (the seed-phase notification channel is the user's mail; mechanism recorded in the runner config, not this document).

## 9. Bootstrap, environment, migration

- Phases 0 (tagged, ungated, logged founding commit) → 1 (shadow) → 2 (blocking) as v2, with one change: **`nc-bootstrap-N` is a standing recovery shape, not a one-shot** — a human-signed, path-unrestricted, tagged, logged commit lane reserved to the human authority (§11), each use requiring a follow-up audit finding. Phase 0 is simply `nc-bootstrap-0`.
...
- **Waivers bind to `{rule_ids, fingerprint set, policy_bundle_digest}` — never a tree.** They are recorded through the `nc-bootstrap-N` lane (human-signed, ungated by construction — the escape hatch does not pass through the machinery it escapes), expire ≤ 7 days, and always open a follow-up finding. The reducer honors an unexpired matching waiver as an accepting verdict.
- Broken-gate repair: self-blocking key class → §7's candidate-value path; anything worse → `nc-bootstrap-N`. Both leave ledger records and follow-up findings.
...
Each entry: the attack, what the record captures, the audit that finds it, and the upgrade trigger. Standing audit cadence: seed phase = the user's walk of merges; automated spot-audits join the repo-health lane at CI adoption.
