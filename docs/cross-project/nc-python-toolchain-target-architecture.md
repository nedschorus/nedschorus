...
- **Escalation clock:** a finding's `first_seen` is carried in ledger lines (`first_seen_carryover`) from the first run that reported it; `nc-checkin-quality-gate pending` reads it from the working tree's ledger. Pending > 24 h → the runner notifies the human authority (the seed-phase notification channel is the boss's mail; mechanism recorded in the runner config, not this document).
