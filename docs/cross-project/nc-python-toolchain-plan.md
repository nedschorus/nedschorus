...
2. **Tool configs** (settled; the Ruff block inlined 2026-07-31 as the union of the two reviewed candidate lists, which previously existed only in the machine-local session archive): mypy strict with relaxed-`Any` overrides for `tests.*` and `nc_checkin_quality_gate.*` (the checker's own package); pytest with `--disable-socket`, `--timeout=30`, `xfail_strict`. Environment: uv, committed hash lockfile, `.python-version` = 3.13.x, `uv sync --frozen` as the one repair command. The Ruff block, verbatim (pin verified on PyPI 2026-07-31 — 0.16.1 is the current latest):
