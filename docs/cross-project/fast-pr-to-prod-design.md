...
| Entry manifest | Quarry imports run through the throat's import command, which in one transaction copies the component, computes source and destination content digests, and WRITES the `entry-manifest.md` line itself — injected, not remembered (adopted 2026-07-21 from the prior-art cross-check `docs/wiki/prior-art-cross-check.md`: Google third-party first-change bundling, Mozilla executable vendoring manifests, Buildroot digests). Line fields: source revision and paths, content digest, destination paths, admission date, purpose, why existing successor code was insufficient, transformations applied. One component per landing; every import is no-upstream-sync (the quarry is frozen). The check refuses a landing whose DECLARED quarry imports lack their matching manifest line in the same landing. Residual: an import done wholly by hand outside the import command remains mechanically undetectable — narrowed from before, and caught by the boss's reading as manifest-less quarry-shaped code. |
