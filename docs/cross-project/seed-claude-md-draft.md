...
# Seed CLAUDE.md — draft for the user's line-by-line admission

**This is a DRAFT of the successor project's CLAUDE.md.** Every line below must be individually admitted by the user before it ships; cut freely. The distillation rule: only lines that change behavior and cannot be a mechanism yet. Everything else lives in the charter, the legacy system, or code.
...
## Who and where
- One human (the user) + one agent. The user reads every checked-in document; there is no automated review, publishing, or merge process — and none gets added without his explicit admission, following the ladder: manual → script-you-run → automation, each step earned by evidence.
...
- Optimize for correctness and clarity over speed. Small increments: one file, one decision at a time, walkable with the user.
...
## Comms
- With the user: this terminal. With the old system: the bridge logs per `comms-bridge-spec.md` (write yours, read theirs); logs are .gitignored — promote anything durable to a committed file.
