...
- **Throat rulings 2026-07-21 (all in the specification):** the account layout — the machine account is choirmaster's credential and the branch lock's sole keyholder; the boss's personal account is repository admin with **no push path** (the boss never commits directly; a boss-originated change lands through the throat like any other; issue participation is unaffected — the repo is public). Check-table additions adopted from the prior-art cross-check: exact-tree validation, trailer-schema provenance record, declared-vs-actual paths, replay protection, and the executable quarry-import command (one-transaction copy + digests + manifest-line write; one component per landing).
- **The publishing program** ([nedschorus#4](https://github.com/nedlern/nedschorus/issues/4)): strategy admitted and walked in full; `nedschorus.com` is **registered** (the boss, 2026-07-21); every further external action — account, site, first post — waits on a per-action go; the first build step is deliberately unpicked while the boss deliberates.
- **The prior-art cross-check is complete** — two adversarial passes over the nine founding patterns; verdicts and every borrowing's disposition at [docs/wiki/prior-art-cross-check.md](../wiki/prior-art-cross-check.md) (the first wiki page). Net: throat/ladder/python-first are established practice (validating, not novel); the handoff governance bundle and quarry-with-manifest survived as unmatched compositions; handoff-quality instrumentation deferred low-priority with raise-triggers ([nedschorus#6](https://github.com/nedlern/nedschorus/issues/6)).
...
| [prior-art-cross-check.md](../wiki/prior-art-cross-check.md) | Wiki page (design-as-of 2026-07-21): two-pass prior-art verdicts on the nine founding patterns; dispositions on closed [nedschorus#5](https://github.com/nedlern/nedschorus/issues/5). |
