...
supersedes: the 2026-09-04 draft, which failed its cold read (236 findings, no clean sections). That draft was never committed; its text and its reports are preserved, machine-local and gitignored, in cold-read-records/2026-09-04-design-to-main-state-machine-design/ on the user's Mac. The 2026-09-05 revision of this document answered a second cold read (347 findings; the sibling directory cold-read-records/2026-09-05-design-to-main-state-machine-design/). This revision applies the rulings of the three walks that followed (2026-09-05/06) and the defects of a third cold read (cold-read-records/2026-09-06-design-to-main-state-machine-design/, whose dispositions.md lists what was applied and what was not, and why).
---
...
- `docs/walk/pr-review-graph-coder-reviewer-originator-loops-minutes.md` — the first walk (2026-09-03 to 04), which produced the rules the superseded draft summarised. Its minutes use the older names *code-design* and *test-code-design* for what this document calls the design and the test-design.
- `docs/walk/state-machine-design-cold-read-triage-minutes.md` — the second walk (2026-09-04 to 05), which triaged the draft's cold read and produced the 2026-09-05 revision. A ruling cited below as "item N" with no walk named is there.
- `docs/walk/state-machine-design-round-2-cold-read-flags-minutes.md` — the third walk (2026-09-05 to 06), on the second cold read; it renamed the investigation and the submission.
- `docs/walk/implementation-review-verdicts-and-code-write-counting-minutes.md` — the fourth walk (2026-09-05 to 06), on what a reviewer can find and what is counted; its item 2 ruled that the design's author writes the component-contract and that the design and the test-design each get an agent's check before the user's.
- `docs/walk/state-machine-design-section-11-open-questions-minutes.md` — the fifth walk (2026-09-06), which closed three questions this document had left open and carries the state-name table (its "SIDE RULING, STATE NAMES").
...
- `initiate-design-to-main` and `design-writing` are conversations with the user; `design-writing` launches a fresh agent to hold its side of the conversation.
- Every sub-state ending in `-acceptance-by-user`, and `investigate-workflow`, wait for the user; `investigate-workflow` launches a fresh arbitrator to talk with him.
- Every sub-state ending in `-acceptance-by-program`, and `submit-to-PR-gate`, run code alone.
- `test-suite-executing` runs code, and launches a single-purpose agent for each test of coverage-type `prompt`.
- `stopped` does nothing; it is the terminal state.
- Every other state launches one fresh agent with its agent-instructions and one state-package.

A launched agent receives one state-package, does one task, emits one state-exit, and ends; the arbitrator may, within its instance, ask the machine to rerun the test suite, which is a service call and not a state-exit (§6.5). A state that waits for the user records his answer as a file before routing on it. A reviewing state is a composite state: its sub-states are the acceptance-checks of the artifact it reviews, run in order (§3.1).
...
**Together the states are one Code Prompt Code (CPC) program**, the fleet glossary's term: code launching agents and reading their state-exits. Its input is a conversation with the user about one component; it ends with a submission accepted by the gate, or with the component stopped. The fleet already has the pattern: `scripts/cold-read-grid.py` is code that launches reviewer agents and reads their status lines.

## 2. Vocabulary

A **component** is what one design describes: one design, one component, one run. A **run** is a component's whole life in the machine, from the user's invocation to `stopped`; an investigation (§6.6) pauses a run, and ends it only when the user rules `stop`. Counters, the topic branch, and the user rulings are per component.
...
**Durable prose** lands where people read it and stays: `docs/`, the wiki, skills, `CLAUDE.md`, and standing-agent-instructions wherever they land. **Run-record prose** is consumed inside one run and stays on the component's topic branch as record. The design and the test-design are durable prose; the component-contract, the notes, and the investigation reports are run-record prose. Where the design and the test-design land after the gate accepts the component is open (§11).

The test suite is **green**, **red**, or **could-not-run** (§6.4).
...
| `contract-reviewing` | `contract-acceptance-by-program`; after a contract-revision also `contract-acceptance-by-agent`; at the revisions ceiling `contract-acceptance-by-user` | the previous version and the notes, on a revision | `advance` / `reject contract` / `discuss` |
...
| `test-suite-executing` | | the implementation; the tests; the test-design | `green` / `red` / `could-not-run` |
| `test-suite-arbitrating` | | the implementation-work-stream's last state-package and files; the test-work-stream's; the whole branch | `advance` / `reject <artifact>` / `flaky-test` / `escalate-to-user` |
...
| `stopped` | | — | none; the terminal state |
...
| `contract-reviewing` | `reject contract` from `contract-acceptance-by-agent`, the counter at its ceiling | `contract-acceptance-by-user` | — |
...
| `design-reviewing` | `reject design` from `design-acceptance-by-agent` | `design-writing` (fresh), with the notes | — (a re-write before approval) |
| `design-reviewing` | `reject contract` from `design-acceptance-by-agent` | `contract-revising` (fresh), with the notes; then `contract-reviewing` and back to `design-reviewing` | — (before approval) |
...
| `implementation-writing` | `input-quick-check-failed` against the component-contract | `contract-revising` | contract-revisions |
...
| `implementation-reviewing` | `reject implementation`, the counter at its ceiling | `test-suite-arbitrating`, which opens the investigation (§7) | — |
| `implementation-reviewing` | `reject contract` | `contract-revising` | contract-revisions |
...
| `test-design-writing` | `input-quick-check-failed` against the component-contract | `contract-revising` | contract-revisions |
...
| `test-design-reviewing` | `reject contract` | `contract-revising` | contract-revisions |
...
| `test-writing` | `input-quick-check-failed` against the component-contract | `contract-revising` | contract-revisions |
...
| `test-reviewing` | `reject tests`, the counter at its ceiling | `test-suite-arbitrating`, which opens the investigation (§7) | — |
...
| `test-reviewing` | `reject contract` | `contract-revising` | contract-revisions |
...
| `test-suite-arbitrating` | `advance` (a rerun was green and the red was the environment's) | `submit-to-PR-gate` | — |
| `test-suite-arbitrating` | `reject implementation`, the counter below its ceiling | `implementation-writing` | — |
| `test-suite-arbitrating` | `reject tests` or `flaky-test`, the counter below its ceiling | `test-writing` | — |
| `test-suite-arbitrating` | any reject whose writer's counter is at its ceiling | `investigate-workflow`, investigation-focus `unknown`, the ruling in the report | — |
| `test-suite-arbitrating` | `reject contract` | `contract-revising` | contract-revisions |
| `test-suite-arbitrating` | `escalate-to-user` | `investigate-workflow`, investigation-focus `unknown` | — |
| `investigate-workflow` | `stop` | `stopped`, outcome `stopped-by-user` | — |
| `investigate-workflow` | `submit-to-PR-gate` | `submit-to-PR-gate` (the user's override; the gate still reviews) | — |
| `investigate-workflow` | `resume`, the redesigns counter below its ceiling or the destination not `design-writing` | the earliest state downstream of what the user changed, by the machine's diff of the branch (§6.6); or the destination the user names; or, nothing changed and no destination, the state that was paused | redesigns, if the destination is `design-writing` |
| `investigate-workflow` | `resume` to `design-writing`, the redesigns counter at its ceiling | `stopped`, outcome `stopped-at-ceiling`; the user is told in the investigation before it closes | — |
| `submit-to-PR-gate` | `accepted` | `stopped`, outcome `completed` | — |
...
- **A redesign resets the version.** Entering `design-writing` from `investigate-workflow` starts a new design version: `tests-begun`, both work-streams' positions, and the implementation-writes, test-writes, contract-revisions and test-design-corrections counters start from zero; the redesigns counter does not.
- **The arbitrator is launched in two states**: `test-suite-arbitrating`, on a red suite, a suite that could not run twice, or a reviewer's reject against a writer whose counter is at its ceiling; and `investigate-workflow`, on every entry (§6.5, §6.6).

### 3.3 The diagram

Derived from §3.2 and not normative where they differ. Mermaid identifiers cannot carry hyphens, so `design-writing` appears as `design_writing`; the names are the same names. The composite reviewing states are drawn with their sub-states; the counter guards, the retry loops and the user's `discuss` returns are not drawn.
...
stateDiagram-v2
    ...
    test_suite_arbitrating --> test_writing: reject tests / flaky-test
    test_suite_arbitrating --> contract_revising: reject contract
    test_suite_arbitrating --> investigate_workflow: escalate-to-user / a writer at its ceiling
    ...
    investigate_workflow --> stopped: stop / redesigns at the ceiling
...
**`contract-acceptance-by-user`** — only at the revisions ceiling: the component-contract has failed review twice, the user sees the original and the first contract-revision with both sets of notes before a second is written, and advances one, discusses, or opens an investigation (§6.6).
...
A state-exit — verdict, destination and state-package commit — and one notes file. The verdicts are `advance`, meaning no material defect whether or not there are unobservable-defects; `reject <artifact>`, naming the artifact under review or an upstream document of it — when more than one is at fault, the furthest upstream, since whatever is downstream of it is rewritten from the corrected version anyway, and when two sibling artifacts are at fault, the implementation and the tests, both are named and both are re-entered; the user's `discuss`; and, for the arbitrator only, `flaky-test` and `escalate-to-user`. The destination is the one §3.2 pairs with the verdict, and the machine checks it. **A reviewer never edits the artifact it reviewed, the component-contract, or the design.** Every fix is done by a fresh writer at the destination.

Notes are evidence for that writer. Each material entry is a finding with a failure scenario and may carry a proposed fix. Unobservable-defects go in a separate section headed as such; the receiving writer's agent-instructions say they are not work. Rejecting an upstream document carries a failure scenario; it need not carry a failing test, because the suite descends from that document.
...
| The component-contract speaks; both the implementation and the tests contradict it | `reject implementation` and `reject tests` | both writers, fresh |
...
| A rerun is green and the red was the environment's | `advance` | `submit-to-PR-gate` |
...
- the arbitrator's `escalate-to-user`, or a reviewer's reject against a writer at its ceiling, where the arbitrator opens it (§7).
...
Once in the investigation the user may edit any file on the branch, ask the arbitrator, or rule. Its exits are `stop`; `submit-to-PR-gate`, the user's override, after which the gate still reviews; and `resume`. On `resume` the machine diffs the branch against the commit at which the investigation opened, ignoring the report and the user-rulings file, and resumes at the earliest state downstream of what changed: the design edited, `design-writing` as a redesign; the component-contract, `contract-reviewing`; the test-design, `test-design-reviewing`; the implementation, `implementation-reviewing`; the tests, `test-reviewing`; nothing edited, the state that was paused. The user may name a destination instead. Any document the user edited is re-read before anything is built from it: the machine starts a cold-read run on it and delivers the cold-read reports to the next writer that receives the document, and the component-contract's program check runs; the user's own acceptance-check of a document he just edited is skipped, the agent's check is not. The `design-review`, `test-design-review` and `contract-review` skills run the artifact's agent check on demand, outside the run, and report to the person who invoked them. A user ruling given in an investigation is appended to the user-rulings file before the run resumes; the latest ruling on a point governs, and a ruling that changes intent is written into the design at the next redesign or by the user's own edit.

**The channel.** The machine opens a session the user can attach to, the way the fleet opens a seat, and the arbitrator does the talking. The arbitrator in `test-suite-arbitrating` may likewise ask the machine to open an investigation when the run looks wrong to it, by `escalate-to-user`. Where the machine runs and how the session reaches the user's Mac is step 1's (§11). The user may also let a run proceed without opening an investigation; his acceptance-checks still wait for him.

## 7. Counting

The machine keeps these counters per component in the run-state file (§9), the four work counters per design version. It bounds the expensive work and the cheap.

| Counter | Increments when | Ceiling | At the ceiling |
| --- | --- | --- | --- |
| **redesigns** | `design-writing` is entered from `investigate-workflow` | two; a third entry is refused | `stopped`, outcome `stopped-at-ceiling` — the user has approved a design and ruled two redesigns, and the machine does not open a third |
| **implementation-writes**, per design version | `implementation-writing` emits `emitted` | three | the third write, when it fails review or the suite, goes to `test-suite-arbitrating`, which opens the investigation, investigation-focus `unknown` |
| **test-writes**, per design version | `test-writing` emits `emitted` | three | the same |
| **contract-revisions**, per design version | `contract-revising` is entered after a rejection or a failed check against the component-contract, once the design is approved | two; the second entry is not made | `contract-acceptance-by-user` |
| **test-design-corrections**, per design version | `test-design-writing` is re-entered after a rejection or a failed check against the test-design, once the test-design is approved | two; the second entry is not made | `test-design-acceptance-by-user` |

A **design version** is the life of one version of the design: the original and up to two redesigns, three versions, so nine implementation-writes in all.
...
Every component has a topic branch, cut by the machine from `origin/main` when the run starts and named for the component, following the topic-branch rule in `docs/issues/238-topic-branch-creation-script-design.md`; the component's name comes from the user's invocation. The branch is a branch of this repository, so the component's artifacts live at the paths the design names for them — a script under `scripts/`, a skill under `.claude/skills/`, its tests where the project keeps tests — and the run's record lives in one directory of its own, so that two components in flight never collide:

| Path | Holds |
| --- | --- |
| the paths the design names | the implementation and the tests |
| `design-to-main-runs/<component>/design.md` | the design, current version |
| `design-to-main-runs/<component>/contract.md` | the component-contract |
| `design-to-main-runs/<component>/test-design.md` | the test-design |
| `design-to-main-runs/<component>/evidence/<sub-state>-<n>/` | a reviewer's notes and its own tests, or a writer's notes, for the nth instance of that sub-state; n is the count of that sub-state's commits in the branch history so far |
| `design-to-main-runs/<component>/reports/investigation-<n>.md` | each investigation report |
| `design-to-main-runs/<component>/user-rulings.md` | every ruling the user has given on this component: approvals, `discuss` outcomes, investigation rulings, each marked user-ruled with its date; append-only, the latest on a point governing |
| `design-to-main-runs/<component>/run-state.json` | the design version; each work-stream's position, or `ready-for-test-suite`; the five counters; `tests-begun`; whether the design and the test-design are approved; the consecutive could-not-run and submit-retry counts; the outcome once `stopped` |

Whether the record directory lands on `main` with the component, or only the artifacts do, is the gate-payload question (§11).

**Every state-exit is committed and pushed by the machine**, in one process, one at a time, whether or not it produced an artifact: an `input-quick-check-failed`, a red suite, an arbitrator's ruling, a user ruling are all commits. The commit message carries a structured trailer — `State:`, `Exit:`, `Package-commit:`, `Write:` (the write number, for writes), and every counter as `Counter-<name>:` — on the model of the gatekeeper's trailer (its design, "The trailer"). The branch history is therefore the record of the run; the arbitrator reads it with `git log` and `git show` in its worktree, and the investigation report cites commits from it. Recovery: a process that dies before committing a state-exit discards the uncommitted files and re-runs that state from the last commit; one that dies after committing and before pushing pushes on restart.
...
**No local repair of an unobservable-defect** (the architecture's §5, "When a nit may be repaired silently"; decision 4's second half, "A safe nit may be repaired, verified, and recorded locally"). A reviewer never edits; unobservable-defects are recorded and never routed (user-ruled 2026-09-06, the third walk, item 6).
...
- **What the gate's pull request carries** — the single candidate commit or the topic branch — and with it whether the run's record directory lands on `main`. The gatekeeper design does not say; merge-lane is routing it.
- **Where the design and the test-design land** after the gate accepts the component, as durable prose. The user's call.
