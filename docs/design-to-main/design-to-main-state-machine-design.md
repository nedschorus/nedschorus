...
| `test-writing` | | the test-design; on re-entry, the tests as they stand and the notes; when an upstream document changed, its diff | `emitted` with coverage-type / `input-quick-check-failed` |
