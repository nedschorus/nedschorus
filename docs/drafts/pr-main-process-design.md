...
| 5 | Sub-walk items 3–6 — `introduced-by`, `basis`, `scope`, and the failure-modes item — carry **no recorded disposition**. The 2026-08-23 rulings on the external value and the conditional fifth tag touch `introduced-by`'s territory, but no source records item 3 as processed | **The user** (walk item) | `phase-4`, sub-walk order and empty Dispositions section |
...
| 17 | The **namespace-prefix question**, named as open by this document's commissioning task. Two greps were run on 2026-08-23 looking for it: `namespace` over `review-fix-cycle-analysis/`, `docs/`, `CLAUDE.md` and `CLAUDE.local.md`, and `prefix` over the three named sources plus `phase-4-research-triage-claude.md` and `-codex.md`. Neither surfaced such a question — the `namespace` hits are the unrelated GitHub/Unix account trap in `docs/issues/3-credential-work-measured-state-and-rulings.md` and the comms-bridge entry-id scheme; the `prefix` hits are conventional-comment prefixes (`Nit:`, Netlify's brackets) quoted in the research reports. Recorded here unresolved because it was named to this drafter as open, and this document does not resolve open questions | **Whoever named it** (the user, or the commissioning seat) | not located in the sources |
