...
The mechanics live in the code — `nc-systems/handoff/handoff-write-and-check-supervisor.py`, `nc-systems/handoff/handoff-supervisor.py`, `scripts/handoff-extract-conversation.py` — so the skill text does not restate them.
...
2. Run `nc-systems/handoff/handoff-write-and-check-supervisor.py --agent <your name> --next-step-file <the file holding your prompt>`. It stamps the time, sets the restart counter, writes the handoff file, and reports whether a supervisor is watching. Add `--dont-restart` only when the user asked to be consulted before a relaunch.
