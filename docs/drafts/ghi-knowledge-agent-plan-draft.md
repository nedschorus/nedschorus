...
**Who performs the lean-split (discussion 2026-08-07, direction set by the user).** The agent already writing or editing the GHI does it, at write time: once the body exceeds a reasonable word count, that agent summarizes the body and creates or updates the linked pair MD. Reasoning: the writer already holds the substance context a rewrite needs — a dedicated rewriter, however broad its project sense, would have to reload what the writer has in hand; and the word count is an observable predicate, so the trigger fits the skill-checklist rule that conditions agents act on must be checkable, and it operationalizes the lifecycle rule's "substantial working material" judgment call. Where the split needs corpus breadth — what to link — the writer asks this knowledge agent, which is the ask-before-writing flow already designed. The knowledge agent's maintenance sweep remains the drift detector: a body grown past the count between edits is flagged, and the next writing agent performs the split as part of its edit. No new agent role is created. The rule, its word-count threshold, and the split procedure ride to the `ghi-write` walk.
