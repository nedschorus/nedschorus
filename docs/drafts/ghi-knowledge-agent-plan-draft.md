...
2. **Is extraction faster or more reliable than querying GitHub live?** Position: the extract's value is neither speed nor reliability — it is *whole-set awareness*. Live search only returns what the asker thought to search for; a curated index the agent holds whole is what lets it answer with issues nobody thought to look for. Delta refresh (verified, `updated:>`) keeps the extract cheap. At today's ~45 issues everything fits trivially; near the 10K bound a one-line-per-issue index strains one context, so tiering (open vs closed) is the named growth point, not built now.
   *Discussion note 2026-08-07 (user): how the agent internally represents the ingested MD is an unknown — there is no control over what it attends to or recalls. The design is a named bet: a modern agent over the whole corpus is simpler and smarter than grep or a vector DB. The bet is testable at build (probe questions with known answers over a loaded corpus), and it is not exclusive: the corpus stays grepable MD on disk, so grep remains one of the agent's own tools; the vector DB is the only thing genuinely not built.*
