...
2. Route by state. Every artifact this skill routes is either at its home or in a named queue with a drain:
   ...
   - Anything carrying pending state — a wanted feature or component, an open question, a commitment to act — gets a GHI (GitHub issue). The issue body stays at 1000 words or fewer. The four cases:
     - **New issue, write-up of 1000 words or fewer:** the body carries the whole write-up. There is no MD.
     - **New issue, write-up longer than 1000 words:** file the issue with the summary as its body — filing is what mints `<n>` — then write the rest into `docs/issues/<n>-<slug>.md`, land it on main through the usual pull request, and edit the body to add the link. (`<slug>`: a few kebab-case words from the title.)
     - **Existing issue that has a pair MD:** new material and corrections go in the MD — revise it there and land it — then re-summarize the body if the summary no longer matches. The body keeps only its summary.
     - **Existing issue with no pair MD:** revise the body. If that takes it past 1000 words, move the whole body into a new `docs/issues/<n>-<slug>.md`, land it, and leave the body a summary and a link.

     (Raised from 500 to 1000 on 2026-08-20: at 500, a body that was nothing but a list of decisions already did not fit.)
