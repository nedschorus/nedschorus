...
All three prompt sets user-ruled and landed: set one (restate) — two cuts plus the frontmatter rule (prose fields restated, data fields skipped), micro-tested twice; set two (defect-hunt) — user-redrafted stem and classes (a)–(e), evidence-restored (f)–(i) with the trigger-on-presence guard, no-severities and document-order output kept, both micro-tests passed (planted defects all caught; benign file drew zero demanded-section findings); set three (primary-agent instructions) — approved verbatim, lives in this design until the grid script encodes it (the script is its home; class (h)'s micro-test deferred to the CLAUDE.md consolidation landing).
