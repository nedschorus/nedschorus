...
- **Piecemeal return (user-directed 2026-08-12):** cells launch as independent background tasks so each completion notifies the requesting agent; triage scores and verifies each report as it arrives rather than waiting for the set. The only barrier is the final cross-cell step (union, dedup, coverage) — everything per-cell processes on arrival.
