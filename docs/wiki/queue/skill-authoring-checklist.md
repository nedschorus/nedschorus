...
- Fix an over-specifying claim by deleting the excess words, not by adding qualifiers — the corrected statement is usually shorter, not longer. Argument for already-ruled decisions is deleted once it is common sense; the decision trail carries the why. (User-ruled 2026-08-09.)
...
- One name, one concept, in both directions: do not rotate synonyms for a thing the skill already named, and do not reuse a name the skill already spent on something else. A reader who meets the same words twice assumes the same referent, and cannot recover when that is wrong. Observed 2026-08-29 in the verify-pass draft, where "the seven" named both a seven-row evidence table and a run's seven findings, leaving a later question pointing at an unresolvable set. (Adjacent but different: CLAUDE.md governs INVENTING a name; this governs using one consistently once chosen.)
