...
4. The zero-context line's "actionable" — record-type artifacts (logs, glossaries, decision records) cannot be actionable; literal compliance forces invented next steps (both good-tier hunts). Proposal: decline or one-word soften; user's call.
5. The decline pile as a class — demands for decision procedures on judgment rules: "standard SDLC terms" needs no glossary; "parts" needs no definition; naming-check search scope, re-collision termination, loop-variable literalism, rename-impossibility, durable-artifact boundary. Proposal: decline all, same reasoning as the self-review's decline pile — cells check logic, not judgment.
