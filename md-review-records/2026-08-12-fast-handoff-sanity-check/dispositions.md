...
4. F4 — mechanize the per-upgrade canary re-run — processed 2026-08-12 → revised: the user chose the finding's alternative, deleting the remembered duty rather than mechanizing the trigger. The user's stated rationale (clarified same day): Claude Code upgrade breakage is a risk he does not care about — not merely a loss made tolerable by containment. The containment exists regardless (the successor's ignition count-check, trial-proven to fire unprompted, with the queues as backstop), and the canaries survive as diagnosis (--canary), no longer as any scheduled duty.
5. (added during item 1) `.claude/settings.json` registers the Stop hook twice under a duplicate `"hooks"` → `"Stop"` JSON key — dedupe — processed 2026-08-12 → resolved externally: a new-vp session had already deduped it on main (b7d7879) the same afternoon; picked up here by merging origin/main, which confirmed one Stop entry and preserved this walk's statusline rename.
