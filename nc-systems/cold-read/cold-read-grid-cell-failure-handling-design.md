...
The design for issue [cold-read grid: retry a failed cold-read-cell once, report its cause, and name a runtime that is down](https://github.com/nedschorus/nedschorus/issues/413). It changes how `nc-systems/cold-read/cold-read-grid.py` handles a cold-read-cell that ends without a report. The cold-read-grid retries the cold-read-cell once and names the cause of each failed attempt. It prints one line when every cold-read-cell of an agent-binary is absent for the same agent-binary-wide cause, and closes every run that finishes with one closing text built from what landed. It carries out the user-ruling of 2026-09-11, whose seven points are quoted in the sections they govern, and his direction of 2026-09-14. The sequence he set on 2026-09-14 is this design, its cold-read-full-run, his review of that cold-read-full-run, and only then code. That review closed on 2026-09-17, and its rulings are in this text.

**Built.** The code landed on 2026-09-18 in pull request [cold-read-grid: a failed cell is retried once and reported with its cause, and every run closes with one closing text](https://github.com/nedschorus/nedschorus/pull/508) (merged as 02f4ede), with the skill text of section 9 in the same pull request, and in pull request [cold-read cells: the classifier skips a leading timestamp, and Codex's captured 401 line is recognised as logged-out](https://github.com/nedschorus/nedschorus/pull/521) (merged as e8698a9). What this document describes is now what the code does. Where it cites code it names the function or constant rather than a line number, because a line number is a copy that drifts; the exception is section 1, whose numbers are pinned to the commit it names and describe code that has since been replaced.

Terms are the glossary's (`docs/nedschorus-wiki/nedschorus-glossary.md`), plus these:

- An **agent-binary** is the installed program a cold-read-cell drives, `claude` or `codex`, recorded as `runtime=` in each report's provenance stamp. The user adopted the name on 2026-09-18, replacing agent-cli (adopted 2026-09-16); its glossary entry and the sweep of the existing files are their own changes, and the stamp field keeps its spelling.
- A **cold-read-cell program** is the launcher script that runs one cold-read-cell: `nc-systems/cold-read/cold-read-claude-cell.py` or `nc-systems/cold-read/cold-read-codex-cell.py`, both built on `nc-systems/cold-read/cold-read-cell-common.py`.
...
- A report has **landed** when its cold-read-cell program exited 0 and the report file exists and is not blank. The test is the condition on the report path in the cold-read-grid's poll loop, unchanged by this design.
...
## 1. What the cold-read-grid did before this design was built

Facts from origin/main at ad9bfca, which is what "today" means in this
section. Line numbers are that commit's, and none of them holds today: the
code in every bullet below was replaced by the code this design specifies.
The section is kept because it is why the change was made.

- **No retry.** Each cold-read-cell program tries the model chain for its tier, and every chain holds a single model (`nc-systems/cold-read/cold-read-claude-cell.py` lines 110-113, `nc-systems/cold-read/cold-read-codex-cell.py` lines 108-111). So a failed model ends the program at once with exit 1, and the `FELL BACK:` line cannot fire. The program exits 64 instead when it refuses its own invocation, before any agent-binary starts.
- **No cause.** A cold-read-cell program that exits non-zero gets `FAILED (exit N): <report file name> (stderr kept: <log>)`, with N the program's exit status. One that exits 0 without a landed report gets `FAILED (exit 0, no report): <report file name> — the cell reported success without writing a review; treat as failed and rerun (stderr kept: <log>)` (`nc-systems/cold-read/cold-read-grid.py` lines 496-501, issue [md-review-grid reports eight reviews saved when eight cells produced nothing](https://github.com/nedschorus/nedschorus/issues/164)). The cold-read-grid's only look at the error is a bare substring test for `401` or `Not logged in` in the log's last 2,000 characters (lines 503-506), which no test covers.
...
- **The cold-read-fast-read already retries.** `nc-systems/cold-read/cold-read-fast-read.py` runs its one cold-read-cell through `nc-systems/cold-read/cold-read-agy-cell.py`, agent-binary `agy`, and retries it once when it fails, except on exit 64, because "the same invocation would be refused the same way". Its test drives the retry with a stub that fails a set number of first attempts (`fail_first_attempts` in `nc-systems/cold-read/tests/cold-read-fast-read-test.py`). The `agy` agent-binary is outside this design.

## 2. The rule every part of this design keeps

**A cause changes what the cold-read-grid reports, never what the cold-read-grid decides.** What it decides is whether a cold-read-cell is retried, whether it is absent, which variant of the closing text prints, and the exit code. Those depend only on whether a report landed, on which attempt it was, and on whether the cold-read-target changed during the run. What a cause changes is what gets reported: the `RETRYING:` and `FAILED` lines, whether a `AGENT-BINARY DOWN:` line prints, what the "Tell the user" sentence names, and what the marker on each report says.

The reason is that causes are read from text, and text can mislead. A cold-read-cell program's log holds the agent-binary's own messages beside the model's text. The Claude limit messages of section 4 arrive on the `claude` agent-binary's standard output, which on a failed attempt also carries whatever the model printed. The Codex agent-binary writes the model's text onto its standard error (recorded in `lift_cell_status_lines` in `nc-systems/cold-read/cold-read-grid.py` and in the classifier's comment in `nc-systems/cold-read/cold-read-cell-common.py`, issue [cold-read grid: FELL BACK and STRAY WRITE markers fire on text that is not the cell's — two false positives measured 2026-09-02](https://github.com/nedschorus/nedschorus/issues/244)). A model reviewing a document that quotes a limit message could print that message at the start of a line.

If a misread cause could skip a retry or change the exit code, that would be a defect. Here it can do less, but not nothing: a misread cause can print a false `AGENT-BINARY DOWN:` line and put a false cause into the "Tell the user" sentence the agent passes to the user. Two things keep that rare and checkable. Each agent-binary's recognised texts are matched only in that agent-binary's own output (section 4). When this was written no Codex text was recognised at all, so Codex output could not be misread; since 2026-09-18 one is, its logged-out line, and it is matched after the leading timestamp the Codex tracing logger puts on every line (section 4). And every cause the user hears carries the path of the kept log it came from (section 6).
...
- Before the relaunch, the cold-read-grid lifts the `STRAY WRITE:` and `WRITE CHECK DID NOT RUN:` lines from the first attempt's log and prints them, as it does today for a landed report. The cold-read-cell program checks for stray writes on its failing path too (`report_stray_writes` in `nc-systems/cold-read/cold-read-cell-common.py`). The retry takes its own baseline after the first attempt's write, so without this step a stray write by a failed first attempt would never be reported.
- It then renames the first attempt's log to `<report file name>.attempt-1.stderr.log`, and the retry writes `<report file name>.attempt-2.stderr.log`. `<report file name>` includes the `.md`, as a single attempt's log is named, as in `2026-09-16-foo--claude-hunt-good.md.attempt-1.stderr.log`. All three names are built from the report path where the cold-read-grid launches and polls its cold-read-cells. A cold-read-cell that is not retried keeps today's single `<report file name>.stderr.log`.
- At the first failure the cold-read-grid prints `RETRYING: <cold-read-cell name> — <cause> (first attempt's log kept: <log>)`. `<cold-read-cell name>` is the agent-binary, pass token and tier the report's file name ends with, as in `claude-hunt-good`. `<cause>` is `<class> — <detail>` (section 4).
...
A retry is not suppressed for a cause a retry cannot fix, such as a logged-out agent-binary. Point 1 retries every failed cold-read-cell, and suppressing a retry would make the cause decide what the cold-read-grid does, which section 2 forbids.

## 4. Naming the cause

The cold-read-cell program, not the cold-read-grid, names the cause of its own failed attempt. It holds the agent-binary's exit status, standard output and standard error separately, before it writes them to its log. As the last line of a failed attempt it prints one status line to its standard error, which the cold-read-grid captures in the attempt's log:
...
The classes, tested in this order; the first that matches names the cause. The first three are recognised in the agent-binary's output. A line matches only by how it starts, never by text found somewhere inside it, the same rule issue [cold-read grid: FELL BACK and STRAY WRITE markers fire on text that is not the cell's — two false positives measured 2026-09-02](https://github.com/nedschorus/nedschorus/issues/244) applies to status lines, and once a line matches by its start the rest of that line is the detail. Each agent-binary's texts are matched only in that agent-binary's output. The matching function lives in `nc-systems/cold-read/cold-read-cell-common.py`, and each cold-read-cell program passes it its own agent-binary's texts. The next three come from the program's own branches and exit status, not from agent-binary text, and the last two are the cold-read-grid's own, named when no program ran to name a cause or when the program left none.

| class | recognised by | detail | what clears it | agent-binary-wide |
|---|---|---|---|---|
...
| `logged-out` | `claude` and `codex`: the line each agent-binary prints when it has no credentials, captured before this class is built (below); the `codex` line after its leading tracing-logger timestamp | the matched line | the user logs in | yes |
| `agent-binary-missing` | the program's `OSError` handler ran when it started the agent-binary (`run_model_chain` in `nc-systems/cold-read/cold-read-cell-common.py`) | the error text, which says whether the binary was not found or would not run | the user installs or repairs the agent-binary | yes |
| `no-report` | the agent-binary exited 0 but no report verified (the program's existing exit-0 branch) | "no report written" | nothing the user does; the retry is the remedy | no |
...
| `exit-N` | anything else; N is the agent-binary's exit status, as in `exit-2` | the last non-empty line of the agent-binary's standard error, or of its standard output when standard error is empty, cut to 120 characters | unknown; the kept log says | no |

Agent-binary-wide means that this cause on one cold-read-cell predicts the same failure for every cold-read-cell of that agent-binary.

No recognised text is guessed. Every text the table recognises must be a real one, captured from an agent-binary, because the tests in section 8 are built from them. The two limit texts were found in the log-store's kept cold-read-cell logs on 2026-09-16. Each is line 2 of its log, on the `claude` agent-binary's standard output.

No logged-out text exists in the log-store, so the agent building this captures one before writing that class. It runs each agent-binary once on ned-box, with an empty configuration directory, so the run fails for want of credentials without touching the real login: `CLAUDE_CONFIG_DIR=$(mktemp -d) claude -p "say hi"` and `CODEX_HOME=$(mktemp -d) codex exec "say hi"`. It records the command, the machine and the output beside the fixture, as the fixture rule requires. If either command does anything but exit non-zero with a message, the builder stops for that agent-binary rather than inventing a text, and that agent-binary's logged-out failure lands as `exit-N` until a real one is captured from a run's log. Today's bare substring test for "401" and "Not logged in" is removed, not carried forward. Codex has no recognised texts either, because none has been captured, so a Codex quota failure lands as `exit-N` until a real one is captured and added. On 2026-09-18 the Codex logged-out line was captured, and it is recognised after the leading timestamp the Codex tracing logger puts on every line, the one exception to column-0 matching the user ruled on 2026-09-18 (walk skill-sentences-and-shipper-questions-2026-09-18, item 4); a Codex quota failure still lands as `exit-N`.

## 5. Saying once that an agent-binary is down

The user's direction of 2026-09-14: "If opus is down, claude is down, so that should be reported."

When a cold-read-cell becomes absent with an agent-binary-wide class, and every other cold-read-cell of its agent-binary has either landed or is absent with the same class, the cold-read-grid prints one line:

`AGENT-BINARY DOWN: claude — account-limit — resets 8:50pm (America/Los_Angeles); 3 reports absent`

The detail is that of the last of them to become absent. Each agent-binary's line prints once, when its condition is met, so when both agent-binaries are down the lines appear in the order the agent-binaries went down. The closing text restates the line in a form that does not start with `AGENT-BINARY DOWN:` (section 6), so the skill's Monitor sees each one once.

It does not print that line in two cases:

- **A model limit is not the agent-binary being down.** On 2026-09-11 the Fable cold-read-cell failed on "You've reached your Fable limit" while both Opus cold-read-cells landed. That is one absent report, class `model-limit`, reported as such. Because `model-limit` is not agent-binary-wide, no `AGENT-BINARY DOWN:` line prints even when every Claude cold-read-cell fails with it; each is listed as absent.
- **Absent reports with different or unrecognised causes are listed one by one** in the closing text. Three `exit-N` failures on one agent-binary are three facts the cold-read-grid cannot join into one.
...
- **The cold-read-target changed:** today's `TARGET_CHANGED_INSTRUCTIONS`, whatever else happened, because a set read against a document that changed during the run is not valid. After it come the absent reports, the agent-binary-down lines and the "Tell the user now" sentence, when there are any.
...
- **Some landed:** "{k} of {n} reports landed in {record_dir}. The set is valid and incomplete: triage the reports that landed." Then the absent reports and the agent-binary-down lines. Then the rest of `COMPLETION_INSTRUCTIONS` after its first sentence, with "until you have read all six" becoming "until you have read every report that landed", and one sentence added: "Record each absent report and its cause in triage.md." The ruling of 2026-09-11 quoted above named that file dispositions.md, which is what it was called then; a cold-read-record's triage file has been triage.md since 2026-09-18, and the sentence that landed, `RECORD_ABSENCES_SENTENCE`, uses the new name.
- **None landed:** "No report landed in {record_dir}, so there is nothing to triage." Then the absent reports and the agent-binary-down lines, and: "Start a new cold-read-full-run once the causes above that the user can clear are cleared, or at once if none of them is."
...
Each absent report is one line: `- <cold-read-cell name>: <class> — <detail> (log: <attempt-2 log path>)`. An agent-binary-down line reads `- claude is down: account-limit — resets 8:50pm (America/Los_Angeles); 3 reports absent`.

When any absent report's class is `logged-out`, `agent-binary-missing`, `account-limit` or `model-limit`, the closing text ends with one sentence: "Tell the user: <causes>." It is the last sentence, after "Start a new cold-read-full-run …" when that prints, and it is omitted when no absent report has one of those classes. One cause reads `<agent-binary> <class> — <detail> (log: <path>)`. Absent reports with the same agent-binary and class share one cause, which takes the detail and log of the last of them to become absent. Causes are joined by semicolons. A Codex quota failure, which lands as `exit-N` (section 4), is not among them.
...
When a report is absent, the cold-read-grid writes one marker line into every `.md` file in the cold-read-record: the reports that landed, and the reference-check file. It uses the mechanism that writes the target-changed marker (`mark_reports_target_changed` in `nc-systems/cold-read/cold-read-grid.py`). That mechanism puts the line after the provenance stamp when there is one, so the stamp stays the first line, and at the top otherwise; this is the top the ruling means.
...
In `nc-systems/cold-read/tests/cold-read-grid-test.py`, every check on the old closing text is rewritten to pin the four variants of section 6. These are the scenarios `checkout-opus-absent`, `checkout-terminology-opus-absent` and `checkout-fable-absent`. They also include `checkout-terminology-codex-absent`, which expects "All six reviews are complete" and "Rerun them singly" after a run with one Codex cold-read-cell absent; `checkout-changed-and-failed`, which expects the `NOTE:` line; and `checkout-stray-write` and `checkout-stray-write-elsewhere`, which check for "All six reviews are complete".

The stub agent-binary the tests use (`STUB_MODEL_RUNTIME` in `nc-systems/cold-read/tests/cold-read-grid-test.py`) kept no state between launches, so it gains per-attempt behaviour. A counter file lets it fail a set number of first attempts, or print different output on each attempt, as `fail_first_attempts` does in `nc-systems/cold-read/tests/cold-read-fast-read-test.py`. New cases:
...
- All three Claude cold-read-cells fail both attempts with `account-limit`. Expect one `AGENT-BINARY DOWN:` line with the reset text, its restatement in the closing text without that prefix, and the reset text in the "Tell the user" sentence.
- All three Claude cold-read-cells fail both attempts with `model-limit`. Expect no `AGENT-BINARY DOWN:` line, and three absent reports listed.
- All three Claude cold-read-cells fail their first attempt with `account-limit`. Two fail their retry the same way, and one fails its retry with `exit-1`. Expect `RETRYING:` naming `account-limit` three times, the third `FAILED` naming `exit-1`, and no `AGENT-BINARY DOWN:` line.
- Three Codex cold-read-cells fail both attempts with `exit-N`. Expect no `AGENT-BINARY DOWN:` line, three absent reports listed, and no "Tell the user" sentence.
...
The classifier gets its own cases in `nc-systems/cold-read/tests/cold-read-cell-common-test.py`. The inputs for the two limit classes are the real lines from the log-store. Each fixture records the log file and line, the command that produced it, which is the cold-read-grid's launch of `nc-systems/cold-read/cold-read-claude-cell.py`, and the machine the run was on:
...
The `logged-out` input is the capture of section 4. `agent-binary-missing`, `no-report` and `exit-N` come from the program's own branches and need no captured text.
...
- **Step 5's Monitor list** gains `RETRYING:` and `AGENT-BINARY DOWN:` (printed as `AGENT-BINARY DOWN:`, below), and "the one marker of the seven that carries no colon" becomes "of the nine". `RETRYING:` means a first attempt failed and the cold-read-cell is being relaunched, which needs no action. `AGENT-BINARY DOWN:` means every cold-read-cell of that agent-binary is absent for one agent-binary-wide cause. The `FAILED` sentence changes to say a cold-read-cell is absent only after its retry failed.
...
Both changes below merged before this design's code, as the order required; the section is kept as the record of what the code waited on.

The code followed two changes queued against the same scripts, in this order. First, pull request [cold-read scripts: sweep comments and docstrings to the glossary's project terms](https://github.com/nedschorus/nedschorus/pull/408), which rewrites comments throughout `nc-systems/cold-read/cold-read-grid.py`. Second, the fixes to the cold-read scripts found by the 2026-09-11 cold-read-full-run of the skill, listed under "Verifiable code defects" in that run's `nedlern@ned-box:/home/nedlern/nedschorus-logs/cold-read-records/2026-09-11-SKILL-2/dispositions.md`. Three of those fixes are in `nc-systems/cold-read/cold-read-grid.py`, and the user ruled on 2026-09-14 that they are merged before this code. They were the MD-skills agent-seat's task 68, never filed as an issue; three of them merged as pull requests 473, 480 and 476, one docstring fix as 481, and one was declined. The skill text of section 9 merged in the same pull request as the code it describes, as this section required.
