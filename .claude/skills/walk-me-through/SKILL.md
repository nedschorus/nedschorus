...
Head every item with the running count — "Item N of M: <the point>" (or "Step N of M" on an innate-order walk) — and lead with the point. M is the current plan's count, purpose item included; a re-plan or size-split restates it. For a finding or recommendation, include the proposed action, not just the problem. End every non-final teaching turn — an item with nothing to rule on — with exactly "Ready for the next item?" (or "Ready for the next step?" on an innate-order walk, or "Ready for the next part?" between sub-steps) — the visible pause is a required slot, not a flourish. A deciding item closes instead on its decision ask, stated in full in the final position: the recommendation or question itself is the pause, and appending the literal pause line after it would give the turn two questions. A turn that answers a question and stays on the current item does not repeat the pause line — re-asking after the user withheld approval reads as pressure; end with the answer.
...
Only the user advances the walk. A clear approval word — "y", "next", "go", "approved", or any equivalent unambiguous yes — moves to the next item, even when it rides alongside a reaction: answer the rider, then advance. On a deciding item, any ruling — an acceptance, a rejection, or a revision — advances the walk once its disposition is captured at the anchor; a comment or question that rules nothing stays on the item. Everything else — any reply with no approval word in it — stays on the current item:
