...
Language: precise standard terminology, plainly used. The user is technical — do not over-simplify — but never coin vocabulary or reach for figurative phrasing where a standard term exists. Write each item in the register of one senior engineer explaining to another out loud: short sentences, concrete nouns, active verbs. When a term of art must appear, restate it in plain words at first use ("the entry checkpoint — the gate that records every legacy import"). Write every item for a reader with no conversation history — the user runs many seats (parallel sessions) and returns after gaps. A sentence that leans on anything said before the walk opened is rewritten to stand alone; an earlier item of the same walk may be leaned on, named with a one-phrase reminder of what it decided. The item's first sentence states its subject as if newly introduced. When the user says he does not understand, rewrite the item as a story built around one concrete failure example — or a concrete worked example when nothing fails — rather than restating the abstraction.

Instruction text an item proposes to adopt — a prompt, a rule, a skill change — gets a zero-context read before the user sees it. The procedure is fixed: launch a fresh subagent that inherits none of this conversation (never a fork), on Sonnet at high reasoning effort, and hand it only the new text plus the project's CLAUDE.md; ask what the text means and where it stumbles — what a reader with no history would misread, which words it cannot resolve. The author is the worst judge of the text's clarity, having watched every sentence form; a reader with no history is the cheapest stand-in for the text's real audience. Fix what the fresh reader misreads before presenting; where the reader's own restatement of the text is clearer than the drafted wording, revise the draft toward the restatement — the author's sentences do not outrank the reader's. Bring any stumble you did not fix to the user with the item.

Fixing any text from findings — the fresh reader's stumbles above, a reviewer's report, or the user's own notes — follows the author discipline the user ruled 2026-08-30: err on the side of simplicity, not complexity. Make corrections and clarifications by simplifying phrases, sentences or paragraphs. Aim for a stable word count. If you believe a correction needs a new section, additional sentences, or new clauses added to existing sentences, enumerate those additions, explain why each is needed, and let the user rule. Fix an over-specifying claim by deleting the excess words, not by adding qualifiers — the corrected statement is usually shorter, not longer (his 2026-08-09 rule, incorporated by name; the full checklist is docs/wiki/queue/skill-authoring-checklist.md). A finding phrased as a question for the user is never answered from your own knowledge: leave the text as it stands and bring the question to him with the item. Never write a reviewer's stated assumption into the text as fact: a guess stays a guess until the user settles it.
