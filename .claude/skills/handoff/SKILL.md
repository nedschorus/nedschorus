...
2. Run `scripts/handoff-write-and-check-supervisor.py --next-step-file <that file>`. Pass no `--agent` unless a refusal tells you to: it defaults to the name your supervisor watches — a session name is not it. Add `--dont-restart` if the user does not want an automatic relaunch.
