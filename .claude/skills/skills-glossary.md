...
This page defines the system-terms of this project's skills: the terms they alone use. Every project-term is defined in the project glossary, `docs/nedschorus-wiki/nedschorus-glossary.md`, and this page uses it as defined there. The entries present on 2026-09-21 were moved here from the project glossary, because no system but the skills uses them (user-ruled 2026-09-21). Cite the program that launches a cold-read-full-run by its file name, `nc-systems/cold-read/cold-read-grid.py`; it has no entry here. The /cold-read skill's programs are the `nc-systems/cold-read/cold-read-*.py` files, and the prompt files of its cold-read-passes are in `.claude/skills/cold-read/prompts/`, with a copy of fast-clarify inside `nc-systems/cold-read/cold-read-fast-read.py`.
...
- **cold-read-full-run** — the /cold-read skill's run of six cold-read-cells, four on the defect-hunt cold-read-pass and two on terminology, launched by `nc-systems/cold-read/cold-read-grid.py`.
