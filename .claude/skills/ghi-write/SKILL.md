...
2. Route by state. Every artifact this skill routes is either at its home or in a named queue with a drain:
   ...
   - Anything carrying pending state — a wanted feature or component, an open question, a commitment to act — gets a GHI (GitHub issue): a GHI-MD when substantial working material rides with it, issue-only otherwise. The body keeps the summary and the GHI-MD takes the substance — design substance, working material, a ruling, a measurement — and the GHI-MD sequence is write the MD, land it on main, then cite it from the issue. **The body has no word limit** (user-ruled 2026-09-19, replacing the 1000-word cap that was raised from 500 on 2026-08-20: the substance is moving to ordinary MD files, so a cap that forces a split no longer earns its cost).
