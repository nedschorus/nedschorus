...
2. Route by state. Every artifact this skill routes is either at its home or in a named queue with a drain:
   ...
   - Anything carrying pending state — a wanted feature or component, an open question, a commitment to act — gets a GHI (GitHub issue): a GHI-MD when substantial working material rides with it, issue-only otherwise, and the issue body stays under 1000 words either way. **A body that would exceed 1000 words splits: never shorten what is already there to make room.** Whatever pushed it over — design substance, working material, a ruling, a measurement — the body keeps the summary and the GHI-MD takes the substance; the GHI-MD sequence is write the MD, land it on main, then cite it from the issue. (The cap was raised from 500 on 2026-08-20: at 500 a body of nothing but rulings did not fit.)
