...
def target_fingerprint(target: pathlib.Path) -> str:
    ...
    while running:
        ...
        for report_path in list(running):
            ...
            if code == 0 and has_report:
                # TWO WARNINGS EXIST ONLY IN THIS LOG, and this is the branch
                # that deletes the log -- so without lifting them out first, the
                # one path where each is produced is the path where it is
                # destroyed. Both are carried onto the grid's own output, which
                # is what the reviewing agent actually reads.
                #
...
def main() -> int:
    ...
    target_before = target_fingerprint(target)
    failures = wait_for_cells(launch_cells(target, record_dir))
    target_after = target_fingerprint(target)
