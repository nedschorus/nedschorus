...
> **processed 2026-07-29 → REJECTED / WITHDRAWN; recorded as an open research thread** (walk item 2, new-vp session b6241858; ruled by the boss in the parallel Mac-app agent discussion of this same section, pasted into this walk). The retire-mechanism candidate — and the successor proposal derived mid-discussion (rules-born-observable at birth plus a deferred usage-audit) — are both withdrawn; no GHI is filed. What survives, per the boss's verdict: the real question is determining what is used frequently versus never used, not removal itself (removal is a cheap ruling once usage evidence exists); start from existing instruments rather than bespoke machinery — test-coverage tooling, sampling profilers, structured logs (e.g. a per-clause identifier in every hook refusal so the session JSONL accumulates a usage ledger), and external signals (GitHub Actions, human reviewers, user complaints), which may be the most important channel; rare-event and error-handling guards are a distinct value class whose never-firing is not evidence of deadness; production app code belongs in the taxonomy; skill-invocation counting is not worth doing (lazy-loaded, no accumulation); prose-rule effects are indirectly observable, and an LLM asked for root cause can point back at the otherwise-invisible instruction, with ablation only as confirmation. Status: "far from being ready for a plan or even a sketch of a plan" — an open research thread, deliberately NOT a candidate skill, NOT a plan, NOT a GHI.
...
1. §2 retire-mechanism: one new candidate GHI, named-deferred.
   *processed 2026-07-29 → withdrawn; see the §2 processed mark.*
