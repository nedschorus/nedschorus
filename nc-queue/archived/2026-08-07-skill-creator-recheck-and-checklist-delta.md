...
4. §3c — the triggering mechanic that makes easy positive trigger cases non-discriminating. — processed 2026-08-08 → revised then accepted: the checklist's single negative-trigger line became three — test only where triggering is genuinely uncertain; positive tests must be substantive (the source's mechanic, so labeled); negative tests are RARELY needed (boss's revision — only where false-triggering is a live risk, chiefly sibling skills), near-miss form unchanged when written.
