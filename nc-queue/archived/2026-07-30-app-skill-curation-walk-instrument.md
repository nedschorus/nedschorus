...
walk-item: queue-and-drafts-drain-2026-09-22 item 7; its own walk held 2026-07-30, marked complete in this note ("Execution (for new-vp at landing) — walk complete 2026-07-31")
dispositions: [rulings enacted in `.claude/settings.json` by commit [curation-walk enactment: NC project settings — disable design/cowork/productivity bundles, skill-level off for engineering's 9 drops (debug survives per ruling) and anthropic-skills' 3 drops; skillOverrides dissolves both mechanism gaps flagged at walk close; new-vp session 641a5905](https://github.com/nedschorus/nedschorus/commit/19d8d4954352ee844ace95782a6c0182b843f0ed) — three bundles disabled and twelve `skillOverrides` entries `"off"`, `engineering:debug` kept, all still on main 2026-09-24; the pinned field inventory and its two sha256 digests stay only here, with no consumer (no routing test exists or is planned); the `engineering:debug` "Eviction" obligation lapsed with GHI [Candidate skill: diagnose-failure — bounded causal debugging with a three-fix escalation stop](https://github.com/nedschorus/nedschorus/issues/21), closed not planned 2026-09-24; nothing lifted]
dropped-by: new-vp, commit [nc-queue landing: app-skill curation-walk instrument from app-session branch 0f05483 (boss-commissioned; 47 skills, 4 groups, pinned inventory digest verified by recomputation) — landed by new-vp session 641a5905](https://github.com/nedschorus/nedschorus/commit/98b779821d176d038fc4b978cf9fc3de515799f5) (archived 2026-09-24 by the queue drain)
---

# App-skill curation walk — instrument (user-commissioned 2026-07-30)
...
## Group A — obvious keep (block rule + veto pass) — processed 2026-07-30 → KEEP, all seven, user-approved, no vetoes
...
- **engineering:code-review** — PR/diff review workflow. processed 2026-07-30 → **DROP** (user-confirmed) — superseded by review-change [#22](https://github.com/nedschorus/nedschorus/issues/22): the NC adaptation is already planned (task 2) from stronger graded donors (OpenAI review-agent spine, Google criteria); this skill adds nothing as donor and squats on #22's trigger territory. Interim reviews stay user-walked. Pattern set for Group C: the question is "does an NC sibling's plan already subsume it?"
- **engineering:testing-strategy** — test strategy/plan design. processed 2026-07-30 → **DROP** (user-approved) — subsumed by write-test-plan [#18](https://github.com/nedschorus/nedschorus/issues/18) (first scheduled pull; contract + dogfood + taxonomy + riders all exist); this skill is the weakest-graded capability class (generic strategy prose, no oracle/red discipline) squatting on the highest-value NC trigger territory. Nothing survives as donor.
- **engineering:debug** — structured debugging workflow. processed 2026-07-30 → **KEEP with named eviction** (user-approved) — the one real interim window: diagnose-failure [#21](https://github.com/nedschorus/nedschorus/issues/21) builds only "when the project has a diagnosis task" while the gatekeeper build will produce real bugs before then; no doctrine conflict (same donor family; the breaker ladder counts fix attempts from outside any debugging procedure); collision cost zero until #21 boots. Eviction: #21's near-miss routing tests run with this skill present; the loser leaves.
- **engineering:architecture** — ADR creation/evaluation. processed 2026-07-30 → **DROP + extract** (user-approved) — the design-evaluation half is subsumed by design-change [#17](https://github.com/nedschorus/nedschorus/issues/17); the ADR *form* (the real complement — NC's append-only decisions layer needs exactly this shape) extracted to `docs/nedschorus-wiki/queue/adr-form-extract.md` for the md-write build or wiki doctrine, with a claims-line addition per the SSOT rulings.
- **engineering:system-design** — system/API/data-model design. processed 2026-07-30 → **DROP** (user-ruled) — fully subsumed by design-change [#17](https://github.com/nedschorus/nedschorus/issues/17) (code-architect spine + user terminal states); no extractable remainder; vaguest trigger in the bundle, the exact class the description-budget finding warns degrades routing.
- **engineering:tech-debt** — debt audit/prioritization. processed 2026-07-30 → **DROP** (user-ruled) — contradicted by ruled doctrine, not merely subsumed: the accumulate-then-triage model lost to detect-at-the-mechanism (usage/posits [#35](https://github.com/nedschorus/nedschorus/issues/35), design-change sweep, patch-cycle tripwire); every legitimate concern has a stronger NC owner.
- **engineering:documentation** — docs/README/runbook writing. processed 2026-07-30 → **DROP** (user-ruled) — subsumed by the imminent md-write founding build (step 1, skills-first): fresh-reader discipline and artifact-lifecycle rules own this whole territory; genre checklists are generic filler, no extract.
- **engineering:deploy-checklist** — pre-deploy verification. processed 2026-07-30 → **DROP** (user-ruled, C8-C10 block) — no deploy surface by explicit ruling (scope ends at main); revisit trigger = first long-running process (same trigger gating release-transition).
- **engineering:incident-response** — incident triage/postmortem. processed 2026-07-30 → **DROP** (user-ruled, C8-C10 block) — no production surface; the blameless mechanism already sharper in the interrogation protocol (#20 packet); revisit at first real incident, re-judged against learn-from-failure, never restored by default.
- **engineering:standup** — standup generation from activity. processed 2026-07-30 → **DROP** (user-ruled, C8-C10 block) — no standup audience; state reporting served structurally by handoff + queue-depth scrub; no revisit trigger (a one-off ask, not a skill).
...
- **loop / schedule / run / dataviz / artifact-design / artifact-capabilities / claude-api / keybindings-help / fewer-permission-prompts** — operational app machinery. KEEP (no action available) — harmless, occasionally used. Name collision noted: `schedule` (built-in) vs `anthropic-skills:schedule` — one more reason the anthropic-skills copy adds nothing; it rides Group B's veto pass if the user wants it dropped too.
...
**Mechanism gaps as flagged at walk close (superseded):** (1) `engineering` bundle — 9 drops but debug keeps; if per-skill disable is unavailable, recommendation: disable the whole bundle and let debug's eviction arrive early (#21's donor material lives in the source-evidence archive; the 9 drops outweigh one interim keep) — the user confirms at landing. (2) `anthropic-skills` bundle — keeps outweigh its three drops (morning, setup-cowork, schedule); bundle stays enabled, the three ride as ruled-dropped-with-mechanism-gap (harmless per the lazy-load ruling; revisit if per-skill disable exists).
