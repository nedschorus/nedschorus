...
**Mechanism gaps, boss-visible:** (1) `engineering` bundle — 9 drops but debug keeps; if per-skill disable is unavailable, recommendation: disable the whole bundle and let debug's eviction arrive early (#21's donor material lives in the source-evidence archive; the 9 drops outweigh one interim keep) — boss confirms at landing. (2) `anthropic-skills` bundle — keeps outweigh its three drops (morning, setup-cowork, schedule); bundle stays enabled, the three ride as ruled-dropped-with-mechanism-gap (harmless per the lazy-load ruling; revisit if per-skill disable exists).
