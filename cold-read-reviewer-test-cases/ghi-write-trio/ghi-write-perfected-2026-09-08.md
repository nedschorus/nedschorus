...
2. Route by state. Every artifact this skill routes is either at its home or in a named queue with a drain:
   - Material whose fate is not yet decided goes to a queue, not to an issue: `docs/wiki/queue/` for wiki-bound doctrine, `docs/issues/queue/` for pair-bound MDs (Markdown documents); a candidate issue queues as a `draft`-labeled issue — the label is the issue world's queue membership.
   - Anything carrying pending state — a wanted feature or component, an open question, a commitment to act — gets a GHI (GitHub issue): an MD-GHI pair when substantial working material rides with it, issue-only otherwise, and the issue body stays under 1000 words either way — a body that cannot is carrying pair material. (Raised from 500 on 2026-08-20: at 500 a body of nothing but rulings did not fit, and the remedy — split to a pair MD and land it on main — routes decision text into a pull request, which is the most expensive way this project handles prose. 1000 still binds on genuine design substance and working material, which is what the pair split is for.) The body carries the summary, the pair MD the substance; the pair sequence is file the issue, write `docs/issues/<number>-<slug>.md`, land it on main, then cite it from the issue body.
...
4. Write for a zero-context reader. Before submitting, check the three tests: the subject is identifiable from the issue alone (the body plus what it cites); the why is stated; the next action is one a reader who was not in this conversation can take.
5. Make every reference openable and every claim checked:
   - A file reference must open from the reader's seat: full URLs for anything outside this repository; in-repo paths verified present on main before citing — a file still on a branch is landed first, the order the pair sequence takes.
...
- Queue routing: write the queue file under its destination directory, or add the `draft` label to the issue; the drain process is [nedschorus#24](https://github.com/nedschorus/nedschorus/issues/24). Promotion moves the queue file rather than copying it: `git mv` it to its home, the move commit being the record — promote is one of the four drain outcomes (promote, edit, demote, drop) ruled there.
